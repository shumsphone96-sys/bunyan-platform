(()=>{
  ...
  const setOpen=open=>{
    ...
    menu.setAttribute('aria-label',open?(window.BunyanI18n?.getLanguage?.()==='en'?'Close menu':'إغلاق القائمة'):(window.BunyanI18n?.getLanguage?.()==='en'?'Open menu':'فتح القائمة'));
  };
  ...
  nav.addEventListener('click',e=>{if(e.target.closest('a,#adminBtn'))setOpen(false);});

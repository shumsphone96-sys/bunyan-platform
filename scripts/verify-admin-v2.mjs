...
const requiredFiles = [
  ...
  'admin-v2/finance.js','admin-v2/finance.css','admin-v2/news.js','admin-v2/news.css',
  'admin-v2/users.js','admin-v2/users.css','admin-v2/audit.js','admin-v2/audit.css',
  'admin-v2/backup.js','admin-v2/backup.css','admin-v2/settings.js','admin-v2/settings.css',
  'backend/src/bootstrap.js','backend/src/bootstrap-v2.js','backend/package.json',
  'scripts/smoke-admin-v2-preview.mjs'
];
for (const file of requiredFiles) {const content=await readFile(file,'utf8');if(!content.trim())throw new Error(`${file} is empty`)}
...
const previewSmoke=await readFile('scripts/smoke-admin-v2-preview.mjs','utf8');
...
for(const [name,assets,view] of [['Finance',['./finance.css','./finance.js'],'finance'],['News',['./news.css','./news.js'],'news'],['Users',['./users.css','./users.js'],'users'],['Audit',['./audit.css','./audit.js'],'audit'],['Backup',['./backup.css','./backup.js'],'backup'],['Settings',['./settings.css','./settings.js'],'settings']]){for(const asset of assets)if(!html.includes(asset))throw new Error(`${name} asset is not wired: ${asset}`);if(!html.includes(`data-view="${view}"`))throw new Error(`${name} navigation entry is missing`)}
...
for(const route of ['/api/v2/finance/entries','/api/projects'])if(!financeJs.includes(route))throw new Error(`Missing API route in finance workspace: ${route}`);
...
if(financeJs.includes('Number(data.project_id)'))throw new Error('Finance workspace must preserve project UUIDs');
for(const signature of ["app.get('/api/v2/finance/entries',auth","app.post('/api/v2/finance/entries',auth","app.patch('/api/v2/finance/entries/:id',auth","app.delete('/api/v2/finance/entries/:id',auth"])if(!bootstrapV2.includes(signature))throw new Error(`Missing V2 finance API contract: ${signature}`);
if(!bootstrapV2.includes('ALTER COLUMN project_id TYPE text'))throw new Error('Finance project UUID migration is missing');
...
const health=await fetch(`${productionOrigin}/health`,{signal:AbortSignal.timeout(15000)});if(!health.ok)throw new Error(`API health failed with ${health.status}`);const healthBody=await health.json();if(!healthBody.ok)throw new Error('API health payload did not report ok=true');
for(const protectedRoute of ['/api/dashboard','/api/help/requests','/api/requests','/api/finance/entries','/api/audit-logs']){const response=await fetch(`${productionOrigin}${protectedRoute}`,{signal:AbortSignal.timeout(15000)});if(response.status!==401)throw new Error(`Protected deployed route ${protectedRoute} must return 401 without a token, got ${response.status}`)}

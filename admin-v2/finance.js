const F_ORIGINS=['https://api.bunyan-sudan.org','https://bunyan-api-qhkf.onrender.com'];
const F_TOKEN='bunyan_v2_token',F_ORIGIN='bunyan_v2_origin';
const f$=s=>document.querySelector(s);
const fEsc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const fDate=v=>v?new Date(v).toLocaleDateString('ar-SD',{dateStyle:'medium'}):'—';
const fToday=()=>new Date().toISOString().slice(0,10);
const fMoney=(v,c)=>`${Number(v||0).toLocaleString('ar-SD',{maximumFractionDigits:2})} ${c||''}`;
let financeRows=[],financeProjects=[],financeFilter='all';

async function fApi(path,options={}){
  const token=sessionStorage.getItem(F_TOKEN)||'';
  const preferred=sessionStorage.getItem(F_ORIGIN)||F_ORIGINS[0];
  const headers={...(options.headers||{})};
  if(token)headers.Authorization=`Bearer ${token}`;
  let requestOptions={...options,headers};
  if(requestOptions.body&&!(requestOptions.body instanceof FormData)){
    headers['Content-Type']='application/json';
    requestOptions={...requestOptions,headers,body:typeof requestOptions.body==='string'?requestOptions.body:JSON.stringify(requestOptions.body)};
  }
  let lastError;
  for(const origin of [preferred,...F_ORIGINS.filter(x=>x!==preferred)]){
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),90000);
    try{
      const response=await fetch(origin+path,{...requestOptions,cache:'no-store',signal:controller.signal});
      const text=await response.text();let data={};
      try{data=text?JSON.parse(text):{}}catch{data={error:text}}
      if(!response.ok){const e=new Error(data.error||`فشل الطلب (${response.status})`);e.status=response.status;throw e}
      sessionStorage.setItem(F_ORIGIN,origin);return data;
    }catch(e){lastError=e;if(e.status&&e.status<500)break}finally{clearTimeout(timer)}
  }

...
function statusEditor(view,row){if(!['help','participation'].includes(view))return '';const options=view==='help'?[['new','جديد'],['review','قيد المراجعة'],['approved','معتمد'],['rejected','مرفوض'],['completed','مكتمل']]:[['new','جديد'],['review','قيد المراجعة'],['accepted','مقبول'],['rejected','مرفوض'],['completed','مكتمل']];return `<div class="record-actions"><select data-status>${options.map(([value,label])=>`<option value="${value}" ${row.status===value?'selected':''}>${label}</option>`).join('')}</select><button class="primary compact" data-save-status data-id="${esc(row.id)}" data-view="${view}">حفظ الحالة</button><span class="inline-message" role="status"></span></div>`}
...
async function loadDashboard(){const content=$('#content');content.innerHTML='<div class="loading">جاري تحميل المؤشرات...</div>';const [dashboard,help,participation]=await Promise.all([request('/api/dashboard'),request('/api/help/requests'),request('/api/requests')]);const newHelp=Array.isArray(help)?help.filter(x=>x.status==='new').length:0;const newParticipation=Array.isArray(participation)?participation.filter(x=>x.status==='new').length:0;$('#helpBadge').textContent=newHelp;$('#participationBadge').textContent=newParticipation;content.innerHTML=`<div class="kpis"><article class="kpi"><span>المشروعات</span><strong>${esc(dashboard.projects||0)}</strong></article><article class="kpi"><span>المستفيدون</span><strong>${esc(dashboard.beneficiaries||0)}</strong></article><article class="kpi"><span>المتطوعون</span><strong>${esc(dashboard.volunteers||0)}</strong></article><article class="kpi"><span>طلبات المساعدة الجديدة</span><strong>${newHelp}</strong></article><article class="kpi"><span>طلبات المشاركة الجديدة</span><strong>${newParticipation}</strong></article><article class="kpi"><span>التبرعات</span><strong>${esc(dashboard.donations||0)}</strong></article></div>`}
...
async function saveStatus(button){const record=button.closest('.record');const message=record.querySelector('.inline-message');const status=record.querySelector('[data-status]').value;const view=button.dataset.view;const path=view==='help'?`/api/help/requests/${button.dataset.id}`:`/api/requests/${button.dataset.id}`;try{button.disabled=true;message.textContent='جاري الحفظ...';await request(path,{method:'PATCH',body:{status}});message.textContent='تم الحفظ';await load(view)}catch(error){message.textContent=error.message;button.disabled=false}}
async function load(view=state.view){state.view=view;$('#pageTitle').textContent=titles[view]||view;document.querySelectorAll('#adminNav [data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view));$('#content').innerHTML='<div class="loading">جاري التحميل...</div>';closeMenu();try{if(view==='dashboard')await loadDashboard();else await loadList(view)}catch(error){if(error.status===401){clearSession();showLogin('انتهت الجلسة. سجّل الدخول من جديد.');return}$('#content').innerHTML=`<div class="error">${esc(error.message)}</div><button class="ghost" id="retryButton">إعادة المحاولة</button>`;$('#retryButton')?.addEventListener('click',()=>load(view))}}
$('#loginForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget;const message=$('#loginMessage');const button=form.querySelector('button[type="submit"]');if(!form.reportValidity())return;try{button.disabled=true;message.textContent='جاري التحقق...';const result=await request('/api/auth/login',{method:'POST',body:{email:form.elements.email.value.trim(),password:form.elements.password.value}});state.token=result.token;sessionStorage.setItem('bunyan_v2_token',result.token);form.reset();showApp();await load('dashboard')}catch(error){message.textContent=error.message}finally{button.disabled=false}});
$('#adminNav').addEventListener('click',event=>{const button=event.target.closest('[data-view]');if(button)load(button.dataset.view)});$('#content').addEventListener('click',event=>{const button=event.target.closest('[data-save-status]');if(button)saveStatus(button)});$('#menuButton').addEventListener('click',()=>$('#sidebar').classList.contains('open')?closeMenu():openMenu());$('#backdrop').addEventListener('click',closeMenu);$('#refreshButton').addEventListener('click',()=>load(state.view));$('#logoutButton').addEventListener('click',()=>{clearSession();showLogin('تم تسجيل الخروج.')});

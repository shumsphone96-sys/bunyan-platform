...
const TEST_RECIPIENT='249912930540';
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (path === '/club-admin/notifications' && method === 'GET' && response.headers.get('content-type')?.includes('text/html')) {
      ...
      headers.set('x-wadnofei-ui','v22-auto-notifications');
...
async function queueAndSend(env, row, eventType, message){
  ...
  const actualRecipient = String(env.WHATSAPP_MODE || '').toLowerCase() === 'meta_test'
    ? normalizePhone(env.WHATSAPP_TEST_RECIPIENT || TEST_RECIPIENT)
    : targetPhone;

  const prefix = actualRecipient !== targetPhone ? `🧪 اختبار لنظام العضوية\nالمستلم الحقيقي لاحقاً: +${targetPhone}\n\n` : '';

...
function homepageBlock(d){
  ...
  content=content.replace(/<section\b[\s\S]*?<\/section>/g,section=>{
    if(!d.news.length && section.includes('class="v64-news"'))return '';
    if(!d.projects.length && section.includes('class="v64-projects"'))return '';
    if(!d.event && !d.achievements.length && section.includes('v64-duo'))return '';
    if(!d.sponsors.length && section.includes('class="v64-sponsors"'))return '';
    return section;
  });
  return content.replace(/<div class="v64-stat"><b>0<\/b><span>[^<]*<\/span><\/div>/g,'');
}

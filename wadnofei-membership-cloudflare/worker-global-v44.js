...
export default {
  async fetch(req,env,ctx){
    ...
    if(p==='/club-admin/cards'){
      ...
      if(gate) return gate;
      return safeCardsPage(env.DB,u.searchParams);
    }
    ...
    if(x&&m==='POST'){
      const gate=await adminGate(req,env,ctx);
      if(gate) return gate;
      return safeIssueCard(env.DB,Number(x[1]));
    }

    x=p.match(/^\/member-card\/([A-Za-z0-9_-]+)$/);
    if(x&&m==='GET') return safeMemberCard(env.DB,x[1]);
    ...
    if(req.method==='GET'&&ct.includes('text/html')){
      ...
      const h=new Headers(r.headers);h.delete('content-length');h.set('cache-control','no-store');h.set('x-wadnofei-ui','v44-safe-cards');
...
async function adminGate(req,env,ctx){
  try{
    ...
    if(probe.status>=300&&probe.status<400){const loc=probe.headers.get('location')||'/login';return new Response(null,{status:303,headers:{location:loc}})}
...
async function safeCardsPage(db,sp){
  ...
  try{apps=(await db.prepare(`SELECT * FROM applications ORDER BY id DESC LIMIT 200`).all()).results||[];apps=apps.filter(x=>!Number(x.member_id||0))}catch(e){warn+='تعذر قراءة طلبات العضوية. '}
  try{members=(await db.prepare(`SELECT * FROM members ORDER BY id DESC LIMIT 200`).all()).results||[]}catch(e){warn+='تعذر قراءة سجل الأعضاء. '}
  const msg=sp.get('msg');
  const body=`${msg?`<div class="note">${e(msg)}</div>`:''}${warn?`<div class="note bad">${e(warn)}</div>`:''}<section class="panel"><h2>طلبات بانتظار إصدار العضوية</h2>${apps.length?apps.map(x=>`<article><div><b>${e(x.name||x.full_name||'بدون اسم')}</b><small>${e(x.app_no||x.application_no||'')} · ${e(x.phone||'')} · الحالة: ${e(x.status||'pending')}</small></div><form method="post" action="/club-admin/applications/${Number(x.id)}/issue-card"><button>اعتماد وإصدار البطاقة</button></form></article>`).join(''):'<p>لا توجد طلبات غير مرتبطة بعضوية.</p>'}</section><section class="panel"><h2>البطاقات الصادرة</h2>${members.length?members.map(x=>`<article><div><b>${e(x.member_no||'—')} — ${e(x.name||'')}</b><small>${e(x.phone||'')} · ${e(x.status||'active')}</small></div>${x.qr_token?`<a class="btn" href="/member-card/${encodeURIComponent(x.qr_token)}">فتح البطاقة</a>`:''}</article>`).join(''):'<p>لا توجد بطاقات بعد.</p>'}</section>`;
  return H(page('اعتماد العضوية والبطاقات',body));
}

async function safeIssueCard(db,id){
  ...
  try{
    ...
    const vals=[tmp,ap.name||ap.full_name||'',ap.phone||'',ap.address||'',ap.dob||'',ap.job||'',ap.member_type||'',ap.notes||'',ap.photo_key||null,token,exp.toISOString()];
    const r=await db.prepare(`INSERT INTO members(member_no,name,phone,address,dob,job,member_type,notes,photo_key,status,qr_token,approved_at,created_at,membership_expires_at,card_issued_at) VALUES(?,?,?,?,?,?,?,?,?,'active',?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,?,CURRENT_TIMESTAMP)`).bind(...vals).run();
    const memberId=Number(r.meta?.last_row_id||0);if(!memberId)throw new Error('no member id');
...
async function safeMemberCard(db,token){
